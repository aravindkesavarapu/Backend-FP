...
public class BankAccountControllerImpl implements BankAccountController {
	...
	public ResponseEntity<Response> addBankAccount(@RequestBody BankAccountEntity bankAccountEntity)
		...
		if(bankAccountService.findByAccoutTypeAndCustomerId(bankAccountEntity.getAccountType().toUpperCase(), bankAccountEntity.getCustomerId()) != null) {
		...
		if (bankAccountEntity1 == null) {
			...
			List<BankAccountEntity> accountEntities = bankAccountService.findByCustomerId(bankAccountEntity.getCustomerId());
			
			r.setValue("Your " + bankAccountEntity.getAccountType() + " account number is " + bankAccountEntity1.getAccNo());
